	...
	nop										mov 20,ct1		; If RAM0[0] was zero or negative, jump to loop exit (Delayed Branch)
	sub							mov alu,a	mov 0,ct2		; CT1 = 20 ; 
								clr a		mov all,mc0		; CT2 = 0 ; Perform RAM0[0] - 1
	mvi #0,mc1												; CT0 = 1 ; Result to RAM0[0] ; 
	;	----------------------------------------------------; CT1 = 21
	;	Move #0 to RAM1[20] as vertex clip count.
	...
	; -----------------------------------------------------------------------
	;	9. Apply clip flags if needed.
	;	It should only DMA out at the CONTINUE step. This is because the portals will apply stacking flip flags.
	;	Next: If RAM3[2] is < 0, jump to UNCLIPPED.
